...
function initializeNetwork() {
    ...
    network.on('dragEnd', function(params) {
        if (params.nodes.length > 0) {
            // Allow physics to settle briefly, then disable
            setTimeout(() => {
                network.setOptions({ physics: { enabled: false } });
            }, 1000);
        }
...
function buildLegend() {
    ...
    // Define the preferred order for taxonomy categories
    const taxonomyOrder = ['FOUND', 'ANLYS', 'PASV', 'TRANS', 'ACFND', 'POWER', 'FREQ', 'FILT', 'OPAMP', 'SIGNAL', 'AUDIO', 'LAB'];

    // Sort groups by taxonomy order, putting unknown groups at the end
