...
KCL states that the algebraic sum of all currents at any node in a circuit equals zero: \(\sum i_k = 0\). This is a direct consequence of conservation of charge — charge cannot accumulate at a junction. Equivalently, the sum of currents flowing **into** a node always equals the sum of currents flowing **out**.
</div>
</p>
...
**Key Formula: Kirchhoff's Voltage Law (KVL)**

\[\sum_{k=1}^{n} v_k = 0\]

Travel around any closed loop in any direction — the voltages across all elements must sum to zero. Assign **+** when you travel from − to + through a source or against current through a resistor; assign **−** otherwise. The loop direction you choose doesn't affect the final answer.
</div>
...
**Pro Tip: Choose the Reference Node Strategically**
Pick the node connected to the most branches as your reference (ground). This maximizes the number of node voltages that are directly known from voltage sources, reducing the number of equations you must solve. In a circuit with a battery, the battery's negative terminal is almost always the best reference node — it locks one node to 0 V and the other to the source voltage instantly.
</div>
</p>
</div>
...
**Common Mistake: Forgetting Shared-Branch Currents in Mesh Analysis**
When two mesh currents pass through the same branch in **opposite** directions, the actual branch current is the **difference**, not the sum. Students often write \(I_{branch} = I_1 + I_2\) when it should be \(I_1 - I_2\). Always draw arrows for every mesh current before writing equations, and explicitly check the direction of each mesh current through each shared branch.
</div>
</p>
