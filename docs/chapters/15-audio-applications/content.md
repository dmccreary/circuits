...
**Common Mistake: Thinking Later Stages Don't Matter for Noise**
...
SNR is the ratio of desired signal power to noise power, expressed in decibels: \(SNR = 20\log_{10}(V_{signal}/V_{noise})\ \text{dB}\). A higher SNR means cleaner audio. Professional audio systems target SNR > 90 dB; studio-grade converters exceed 110 dB. The noise floor sets the quietest level the system can reproduce.
</div>
</p>
...
**Key Formula: Thermal Noise Voltage**

\[V_{noise} = \sqrt{4 k_B T R \,\Delta f}\]

At room temperature (T = 290 K), a 10 kΩ resistor in a 20 kHz audio bandwidth produces: \(V_{noise} = \sqrt{4 \times 1.38\times10^{-23} \times 290 \times 10^4 \times 20000} \approx 1.8\ \mu\text{V RMS}\). Keep input resistances low and bandwidths narrow to minimize noise.
</div>
</p>
...
**Pro Tip: Use a Star Ground and Keep High-Current Paths Away from Signal Paths**
Route all ground returns to a single star point on the PCB. Never let the output stage return current share a trace with the preamplifier input ground. A practical rule: run separate ground traces from (1) the input connector, (2) each op-amp stage, and (3) the power supply decoupling capacitors — all meeting at one star point near the power supply. If you hear 60 Hz hum, suspect a ground loop as the first culprit.
</div>
</p>
